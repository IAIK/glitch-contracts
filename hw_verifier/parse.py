...
def parse_arguments():
    ...
    group.add_argument("-s", "--source", dest="verilog_file_paths",
                       ...
                       help="Path of Verilog source file(s)", metavar="VERILOG_FILES")
    ...
    parser.add_argument("-n", "--netlist", dest="netlist_file_path",
                        ...
                        help="Path of output verilog netlist file")
    parser.add_argument("-y", "--yosys", dest="yosys_bin_path",
                        required=False, default=None, type=helpers.ap_check_file_exists,
                        help="Path to a custom yosys binary file")
    parser.add_argument("--log-yosys", dest="log_yosys", action="store_true", default=False, required=False,
                        help="Print output of Yosys synthesis process to logfile (yosys_synth_log.txt)")
...
def yosys_synth(args):
    try:
        ...
        if args.log_yosys:
            ...
            print([[yosys_bin_path, yosys_synth_file_path], subprocess.PIPE])
...
def create_label_template(circuit_json, label_file_path):
    ...
    for cell in module["cells"]:
        ...
        label_file.write("%s:%d:%d: %s\n" % (name, pos, bit, defines.LABEL_CONST))
